...
public class ResourceServlet extends HttpServletBean {
    ...
    /**
     * 私有化资源询址实现 规则：寻找当前插件JAR包中resource目录的资源文件。
     * 
     * @param request
     * @return 静态资源文件在jar包中的文件地址
     */
    ...
    private void writeResourceContent(HttpServletResponse response,
        ...
        try {
            zipIn = new ZipInputStream(new FileInputStream(pluginFile));
            ZipEntry zipEntry = null;
            while ((zipEntry = zipIn.getNextEntry()) != null) {
            	System.out.println("zipEntry.getName()-->"+zipEntry.getName());
                if (zipEntry.getName().equals(resourceUrl))
                    break;
            }
            ...
            while ((bufsize = zipIn.read(buf, 0, buf.length)) != -1) {
